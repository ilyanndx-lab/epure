...
| LM Studio — freeze système avec certains modèles 9B | Reproduit nativement dans l'UI LM Studio elle-même (donc indépendant d'Épure), pas seulement via l'intégration Épure. Démarre vers ~500 tokens de génération. Testé avec contexte réduit à 8K et offload GPU réduit à 20/32 couches (Yoga, iGPU Radeon 840M) — le freeze persiste. Aucune activité disque observée pendant le freeze (exclut le mécanisme de thrashing RAM/pagefile documenté pour l'incident Ollama/FLM du 2026-09-11 — cause différente, non résolue). Pas un bug Épure : éviter ce modèle sur LM Studio sur cette machine. |
